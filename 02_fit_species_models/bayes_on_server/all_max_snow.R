...
dece.mxsn = stan_glmer(cbind(n.obs, 100 - n.obs) ~ asp.e + jja_mean1 + pH + max.dep +
...
dece.mxsn.pred = posterior_predict(dece.mxsn, newdata = dece.valid,
                                   re.form = ~ (1 | plot),
                                   seed = 1020,
                                   draws = 4000)

# Generate summary statistics for posterior draws
dece.pred.summ = dece.mxsn.pred %>%
  t() %>%
  as.data.frame() %>%
  mutate(i = 1:nrow(.)) %>%
  ...
  mutate(sp = 'dece', model = 'max.snow')

### Kobresia

komy.mxsn = stan_glmer(cbind(n.obs, 100 - n.obs) ~ Total_N + pH + max.dep +
                                                   (1 | plot) + (1 | year) + (1 | obsno),
                       family = 'binomial',
                       cores = 4,
                       seed = 13610,
...
# Generate posterior predictions
komy.mxsn.pred = posterior_predict(komy.mxsn, newdata = komy.valid,
                                   re.form = ~ (1 | plot),
                                   seed = 408,
                                   draws = 4000)

# Generate summary statistics for posterior draws
komy.pred.summ = komy.mxsn.pred %>%
  t() %>%
  as.data.frame() %>%
  mutate(i = 1:nrow(.)) %>%
  gather(key = draw, val = pred, -c(i)) %>%
  ...
  summarise(yhat_mean = mean(pred),
            ...
            yhat_q841 = quantile(pred, 0.841),
            yhat_q159 = quantile(pred, 0.159)) %>%
  mutate(sp = 'komy', model = 'max.snow')

### Geum

gero.mxsn = stan_glmer(cbind(n.obs, 100 - n.obs) ~ max.dep + (1 | plot) + (1 | year) + (1 | obsno),
                       family = 'binomial',
                       cores = 4,
                       seed = 1025,
                       data = gero.train)
...
# Generate posterior predictions
gero.mxsn.pred = posterior_predict(gero.mxsn, newdata = gero.valid,
                                   re.form = ~ (1 | plot),
                                   seed = 63000,
                                   draws = 4000)

# Generate summary statistics for posterior draws
gero.pred.summ = gero.mxsn.pred %>%
  t() %>%
  as.data.frame() %>%
  mutate(i = 1:nrow(.)) %>%
  gather(key = draw, val = pred, -c(i)) %>%
  ...
  summarise(yhat_mean = mean(pred),
            ...
            yhat_q841 = quantile(pred, 0.841),
            yhat_q159 = quantile(pred, 0.159)) %>%
  mutate(sp = 'gero', model = 'max.snow')

write.csv(rbind(dece.pred.summ %>% 
                  mutate(loglik = log_lik(dece.mxsn) %>% apply(1, sum) %>% mean()), 
                komy.pred.summ %>% 
                  mutate(loglik = log_lik(komy.mxsn) %>% apply(1, sum) %>% mean()), 
                gero.pred.summ %>% 
                  mutate(loglik = log_lik(gero.mxsn) %>% apply(1, sum) %>% mean())),
          row.names = FALSE,
          file = 'output/all_max_snow_summary.csv')

save(dece.mxsn, komy.mxsn, gero.mxsn, file = 'output/all_max_snow.RData')
